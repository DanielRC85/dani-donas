...
// Tus arrays `rellenas` y `especiales` completos...
...
export default function Home() {
  ...
  return (
    <main className="bg-[#050d1a] min-h-screen text-white overflow-hidden flex flex-col items-center">
      ...
      {/* 🚀 CORRECCIÓN DE ANCLAJE: Toda sección tiene w-full flex justify-center */}
      <section className="py-12 md:py-16 w-full flex justify-center">
        <div className="max-w-6xl w-full px-5">
          <div className="mb-10">
            <p className="text-xs font-bold uppercase tracking-widest mb-2 text-cyan-400">&lt;/&gt; Las clásicas</p>
            <div className="flex items-end gap-4 flex-wrap">
              <h2 className="text-3xl md:text-4xl font-black text-white leading-tight">Rellenas 🍩</h2>
              <span className="text-sm font-bold px-4 py-1.5 rounded-full mb-1 bg-yellow-400/10 border border-yellow-400/40 text-yellow-400">
                $5.750 c/u · caja x4 = $23.000
              </span>
            </div>
            <p className="mt-2 text-sm text-gray-400">Hechas frescas · También disponibles sin relleno</p>
          </div>
      ...
      <section className="py-16 md:py-20 w-full flex justify-center">
        <div className="max-w-6xl w-full px-5">
          <div className="mb-12 flex flex-col items-center text-center">
            <p className="text-xs font-bold uppercase tracking-widest mb-2 text-pink-500">🔥 Promo inauguración</p>
            <div className="flex flex-col md:flex-row items-center justify-center gap-3 md:gap-5 flex-wrap">
              <h2 className="text-3xl md:text-4xl font-black text-white leading-tight">Especiales ✨</h2>
              <div className="flex items-center gap-3 mb-1">
                <span className="text-base font-bold line-through text-gray-500">$6.500 c/u</span>
                <span className="text-sm md:text-base font-bold px-4 py-1.5 rounded-full bg-pink-500/15 border border-pink-500/50 text-pink-500">
                  HOY $5.750 c/u 🔥
                </span>
              </div>
            </div>
          ...
          <div className="mt-20 p-6 md:p-8 rounded-3xl bg-pink-500/5 border border-pink-500/20 text-center">
            <p className="text-base md:text-lg text-gray-300">
              💡 <strong className="text-white font-black">Arma tu caja como quieras:</strong> mezcla rellenas + especiales.{" "}
              <span className="text-pink-400 font-bold block mt-2 md:inline md:mt-0">Todo a $23.000 por inauguración.</span>
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 md:py-32 w-full flex justify-center">
        <div className="max-w-3xl w-full px-5 text-center">
          <p className="text-xs font-bold uppercase tracking-widest mb-3 text-yellow-400">El Proceso</p>
          <h2 className="text-3xl md:text-4xl font-black text-white mb-16">¿Cómo pedir? 📲</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-6">
            {[
              ...
              <div key={step} className="flex flex-col items-center gap-5">
                <div className={`w-16 h-16 rounded-full flex items-center justify-center font-black text-xl ${bg} ${border} border ${color}`}>
                  {step}
                </div>
                <p className="text-base text-gray-300 font-medium">{text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-5 text-center relative overflow-hidden w-full flex justify-center">
        ...
        <div className="relative z-10 max-w-2xl w-full">
          <h2 className="text-3xl md:text-5xl font-black text-white mb-4 leading-tight">No te quedes sin <br className="md:hidden" /> probarlas 😏</h2>
          <p className="mb-10 text-base md:text-lg text-gray-400">Promo de inauguración · Unidades limitadas todos los días</p>
          <a href={promoLink} target="_blank" rel="noopener noreferrer" className="group relative inline-flex items-center justify-center gap-3 px-12 py-5 rounded-2xl font-black text-lg text-white overflow-hidden transition-all duration-300 hover:scale-105" style={{ background: "linear-gradient(135deg, #ff2d78 0%, #ff00ff 50%, #7c3aed 100%)", boxShadow: "0 0 30px rgba(255,45,120,0.4)" }}>
      ...
      <footer className="text-center py-10 text-xs md:text-sm text-gray-500 border-t border-white/5 bg-black/40 mt-10 w-full flex flex-col items-center">
        <p className="mb-2">© {new Date().getFullYear()} Dani Donas · Medellín, Colombia</p>
        <p className="tracking-widest uppercase font-bold text-pink-500/70">Programadas para enamorar 🍩</p>
      </footer>
    </main>
  );
}